...
# Filter for focused time window
hazaAndTime = df[(df['rcvTime'] >= 2400) & (df['rcvTime'] <= 3200)]
hazaAndTime.plot(figsize=(20, 10))
plt.title("Focused Hazard Attack View")
plt.show()


######################################### Part 2: Time-Series Preparation and Splitting

# Convert time-series into supervised learning format
def df_to_X_y(df, window_size=60, step=1):
    df_np = df.to_numpy()
    X, y = [], []
...
######################################### Part 3: LSTM Model Construction and Training
...
# Plot training predictions vs actuals
...
# Optional: Export predictions to CSV for analysis or reporting
...
test_results.to_csv("lstm_test_predictions.csv", index=False)
...
# model.save('final_lstm_model.h5')  # Uncomment if needed
